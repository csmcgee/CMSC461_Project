  ...
  private ActionListener orderBtnEvent(){
    return new ActionListener(){
      ...
      public void actionPerformed(ActionEvent arg0) {
        ...
        String orderType = (String)view.getShippingField().getSelectedItem();
        try {
          StudentCartModel.createOrderFromBooks(student.id, books, cc, orderType, total);
        } catch (Exception e) {
          e.printStackTrace();
          JOptionPane.showMessageDialog(view, "Oops. Something went wrong with the order.");
        }
        view.repaint();
  ...
  private ActionListener addToCartEvent(){
    return new ActionListener(){
      ...
      public void actionPerformed(ActionEvent arg0) {
        ...
        try {
          StudentCartModel.addToCart(student.id, book.ISBN, book.type,
              book.status, book.format, book.quantity);
        } catch (SQLException e) {
          JOptionPane.showMessageDialog(view, "Book does not exist. Please check that all inputs correspond to a book in the inventory.");
          e.printStackTrace();
        }
        view.repaint();
